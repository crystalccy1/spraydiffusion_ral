...
trajectory, given object point-cloud in input
    ...
    Examples:
        ...
        - Reproduce paper results: 
            ...
            - python submodules/paintnet/train.py --config submodules/paintnet/configs/windows_stable_v1.json --seed 42
"""
import pdb
import sys
import argparse
...
# Import the local metrics_handler, not the one from parent directory
import sys
import os
...
def parse_args():
    ...
    parser.add_argument('--dataset',        default='shelves-v2', type=str, help='Dataset name [containers-v2, windows-v1, shelves-v1, cuboids-v1]')
    ...
    parser.add_argument('--workers',        default=0, type=int, help='Number of   for datasetloader')
...
def main():
    ...
    wandb.init(project="paintnet_v2",
               config=config,
               name=run_name,
               group=args.group,
               save_code=True,
    ...
    time_end = time.time()
    print(f"Time taken to load train dataset: {time_end - time_start} seconds")
    ...
    te_dataset = PaintNetDataloader(root=dataset_path,
                                  ...
                                  split='test')
    time_end = time.time()
    print(f"Time taken to load test dataset: {time_end - time_start} seconds")
    ...
    for epoch in epoch_pbar:
        ...
        # 添加batch级别的进度条
        batch_pbar = tqdm(tr_loader, desc=f"Epoch {epoch+1}/{args.epochs}", 
                         unit="batch", leave=False)
        
        for i, data in enumerate(batch_pbar):
            time_start_batch = time.time()
            # print(f"Time taken to load batch: {time.time() - time_start_batch} seconds")
            model.zero_grad()
            ...
            # 更新batch进度条显示当前损失
            current_avg_loss = tot_loss / data_count
            batch_pbar.set_postfix({
                'Loss': f'{current_avg_loss:.5f}',
        ...
        # 计算epoch平均损失
        epoch_avg_loss = tot_loss * 1.0 / data_count
        
        wandb.log({"TOT_epoch_train_loss": epoch_avg_loss, "epoch": (epoch+1)})
        tot_loss_list = tot_loss_list * 1.0 / data_count
        loss_handler.log_on_wandb(tot_loss_list, epoch, wandb, suffix='_train_loss')
        
        # 更新epoch进度条
        epoch_time = time.strftime("%M:%S", time.gmtime(time.time() - start_ep_time))
        epoch_pbar.set_postfix({
            'Train Loss': f'{epoch_avg_loss:.5f}',
            'Time': epoch_time,
            'Best Loss': f'{best_eval_loss:.5f}' if best_eval_loss != sys.float_info.max else 'N/A'
        })
        
        print(f'[{epoch+1}/{args.epochs}] Epoch time: {epoch_time} | Epoch train loss: {epoch_avg_loss:.5f}')
        ...
        if (epoch+1) % args.eval_freq == 0:
            ...
            if is_best:
                ...
                print(f'🎉 New best model saved! Best loss: {best_eval_loss:.5f}')
...
def test(model, loader, loss_handler, metrics_handler=None, save=False, **save_args):
    ...
    # 添加测试进度条
    split_name = save_args.get('split', 'test').title()
    test_pbar = tqdm(loader, desc=f"{split_name}ing", unit="batch", leave=False)
    ...
    for i, data in enumerate(test_pbar):
        ...
        # 更新进度条显示当前平均损失
        current_avg_loss = tot_loss / data_count
        test_pbar.set_postfix({'Avg Loss': f'{current_avg_loss:.5f}'})
