...
# Add path to main project for pytorch3d_chamfer import
...
# Try to import torch_nndistance, set flag if not available
NND_AVAILABLE = True
try:
    import torch_nndistance as NND   
except ImportError:
    print('Warning! Unable to import torch_nndistance package. Falling back to PyTorch3D chamfer distance.')
    NND_AVAILABLE = False
    NND = None
...
try:
    ...
    print(f'Warning! Unable to import pytorch3d_chamfer package.'\
          f'Chamfer distance with velocities won\'t be available.'\
          f'(Check troubleshooting.txt for info on how to install pytorch3d_chamfer)')
...
class LossHandler():
    ...
    def get_chamfer(self, y_pred, y, **args):
        if chamfer_distance is None:
            raise ImportError("chamfer_distance is not available. Please check pytorch3d_chamfer import.")
