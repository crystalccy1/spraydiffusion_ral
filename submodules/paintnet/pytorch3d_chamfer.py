...
def chamfer_distance(
    ...
    if padded:  # Overwrite y_lengths to handle custom padding of [-100, -100, -100] points
        ...
        if len(ridx) != 0 or len(cidx) != 0:  # whether there actually is any padding
            new_y_lengths = []
            for b in range(N):
                # Check if this batch has any padding values
                batch_mask = (ridx == b)
                if batch_mask.sum() > 0:  # There are padding values for this batch
                    new_y_lengths.append(cidx[torch.argmax(batch_mask.type(torch.IntTensor))].item())
                else:
