...
def from_strokewise_to_pointwise(strokes, config, return_stroke_ids=True, remove_padding=True, device='cpu'):
    ...
    该函数将笔画级别的表示转换为点级别的表示，方便后续处理。
    它可以选择性地返回每个点所属的笔画ID，以及移除填充点（标记为-100的点）。
    
    参数:
        strokes: [N, max_n_stroke_points*outdim] 笔画级别的输入数据
        config: 配置对象，包含额外数据维度信息
        return_stroke_ids: 是否返回每个点对应的笔画ID
        remove_padding: 是否移除填充点（标记为-100的假点）
        device: 计算设备，默认为'cpu'
        
    返回:
        如果return_stroke_ids为True，返回(strokes, stroke_ids)
        否则仅返回strokes
    """
