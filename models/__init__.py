...
# Import DP3 class
try:
    from spray_diffusion.policy.dp3 import DP3
    DP3_AVAILABLE = True
except ImportError as e:
    print(f'Model import error: DP3 is not available. Error: {e}')
    DP3_AVAILABLE = False
    DP3 = None
...
def get_model(config, which, io_type=None, custom_model_config=None, device="cpu"):
    ...
    """
    assert io_type in {None, 'paintnet', 'multipathregression', 'ContrastiveClustering', 'ODv1_strokeProposal', 'ODv1_strokeRollout', 'MaskPlanner', 'StrokeWise', 'SprayDiffusion'}
...
def get_model_diffusion(config, which, io_type=None, custom_model_config=None, device="cpu"):
    """
        ---
        io_type : str
                  input/output network type [paintnet, multipathregression, None]
                  ...
                  - multipathregression: the `stroke_pred` task (train_stroke.py) is instead considered. 
    """
    assert io_type in {None, 'SprayDiffusion'}
    model_config = config.model if custom_model_config is None else custom_model_config  # use the "config.model" params by default
...
def get_raw_model(config, which, io_type=None):
    ...
    if which == 'pointnet': # PointNet
        ...
        if not DP3_AVAILABLE:
            raise ImportError("DP3 model is not available. Please ensure all dependencies are installed.")
            
        # Import required classes
        from diffusers.schedulers.scheduling_ddim import DDIMScheduler
        
        # Configure noise scheduler
        noise_scheduler_config = getattr(config, 'noise_scheduler', {})
        noise_scheduler = DDIMScheduler(
            ...
            num_train_timesteps=noise_scheduler_config.get('num_train_timesteps', 100),
            clip_sample=noise_scheduler_config.get('clip_sample', True),
            set_alpha_to_one=noise_scheduler_config.get('set_alpha_to_one', True),
            steps_offset=noise_scheduler_config.get('steps_offset', 0),
            prediction_type=noise_scheduler_config.get('prediction_type', 'sample')
        )
        
        # Configure shape metadata
        if hasattr(config, 'shape_meta'):
            shape_meta = config.shape_meta
        else:
            # Fallback to default shape metadata if not provided in config
            obs_shape_meta = {'point_cloud': {'shape': [512, 3]}, 'low_dim': {'shape': [24]}}
            shape_meta = {
                'obs': obs_shape_meta,
                'action': {'shape': [24]}
            }
        ...
        return DP3(
            ...
            horizon=io_info['horizon'],
            n_action_steps=io_info['n_action_steps'],
            n_obs_steps=io_info['n_obs_steps'],
            num_inference_steps=getattr(config.diffusion, 'num_inference_steps', 10),
            obs_as_global_cond=getattr(config.diffusion, 'obs_as_global_cond', True),
            diffusion_step_embed_dim=getattr(config.diffusion, 'diffusion_step_embed_dim', 256),
            down_dims=getattr(config.diffusion, 'down_dims', (256, 512, 1024)),
            kernel_size=getattr(config.diffusion, 'kernel_size', 5),
            n_groups=getattr(config.diffusion, 'n_groups', 8),
            condition_type=getattr(config.diffusion, 'condition_type', "film"),
            use_down_condition=getattr(config.diffusion, 'use_down_condition', True),
            use_mid_condition=getattr(config.diffusion, 'use_mid_condition', True),
            use_up_condition=getattr(config.diffusion, 'use_up_condition', True),
            encoder_output_dim=io_info['encoder_output_dim'],
            # Partial observation parameters
            partial_observation_enabled=getattr(config, 'partial_observation_enabled', False),
            partial_observation_method=getattr(config, 'partial_observation_method', 'fixed_camera'),
            partial_observation_ratio=getattr(config, 'partial_observation_ratio', 0.3),
            fixed_camera_positions=getattr(config, 'fixed_camera_positions', None),
            fixed_camera_directions=getattr(config, 'fixed_camera_directions', None),
            # Ablation study parameters
            ablation_prev_traj=getattr(config, 'ablation_prev_traj', 'normal'),
            random_traj_seed=getattr(config, 'random_traj_seed', 123)
        )
...
def get_io_info(io_type, config):
    ...
    """
    if io_type == 'SprayDiffusion':
        info = {}
        inputdim = 3  # 3-dimensional points of the object point-cloud
        outdim = get_dim_traj_points(config.extra_data)
        orient_outdim = get_dim_orient_traj_points(config.extra_data)
        
        # Set the action dimension
        action_dim = config.action_dim if hasattr(config, 'action_dim') else outdim
        
        # Set diffusion model parameters
        horizon = config.horizon if hasattr(config, 'horizon') else config.traj_points
        n_action_steps = config.n_action_steps if hasattr(config, 'n_action_steps') else horizon
        n_obs_steps = config.n_obs_steps if hasattr(config, 'n_obs_steps') else 1
        
        # Set encoder output dimension
        encoder_output_dim = config.encoder_output_dim if hasattr(config, 'encoder_output_dim') else 256
        ...
        print(f'SprayDiffusion: horizon={horizon}, action_dim={action_dim}, n_obs_steps={n_obs_steps}')
        
        return info

    elif io_type == 'ODv1_strokeProposal':
        info = {}
        outdim = get_dim_traj_points(config.extra_data)
        orient_outdim = get_dim_orient_traj_points(config.extra_data)
