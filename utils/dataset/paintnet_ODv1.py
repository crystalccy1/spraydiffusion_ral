...
class PaintNetODv1Dataloader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if index in self.cache:
            ...
            point_cloud, traj, traj_as_pc, stroke_ids, stroke_ids_as_pc, dirname, extra_items,mesh_vertices_np, mesh_faces_np = self.cache[index]
        else:
            # Retrieve from filesystem
            mesh_file, traj_file, dirname = self.datapath[index]
            # self.force_fresh_preprocess = True
            ...
            if not self._preprocessed_sample_exists(mesh_file, traj_file, self.extra_data, self.weight_orient) or self.force_fresh_preprocess:
                ...
                """
                print("No preprocessed sample found")
            ...
            assert self._preprocessed_sample_exists(mesh_file, traj_file, self.extra_data, self.weight_orient) or self.force_fresh_preprocess, "Preprocessed sample does not exist"
                
            # else:
                # Load already-preprocessed sample
            preprocessed_dir = os.path.join(os.path.abspath(os.path.join(mesh_file, os.pardir)), 'paintnet_preprocessed_sample')
            mesh_vertices_np_path = os.path.join(preprocessed_dir, f"mesh_vertices_np_{index}.npy")
            mesh_faces_np_path = os.path.join(preprocessed_dir, f"mesh_faces_np_{index}.npy")
            # mesh_vertices_np_path = os.path.join(preprocessed_dir, f"mesh_vertices_np_original_{index}.npy")
            # mesh_faces_np_path = os.path.join(preprocessed_dir, f"mesh_faces_np_original_{index}.npy")
            ...
            if not os.path.exists(mesh_vertices_np_path) or not os.path.exists(mesh_faces_np_path):
                import trimesh
                ...
                mesh_vertices_np -= np.mean(mesh_vertices_np, axis=0)
                if self.normalization == 'per-dataset':
                    mesh_vertices_np /= self.dataset_mean_max_distance
                elif self.normalization == 'per-mesh':
                    mesh_vertices_np /= get_max_distance(mesh_file)
                np.save(f"{preprocessed_dir}/mesh_vertices_np_{index}.npy", mesh_vertices_np)
                np.save(f"{preprocessed_dir}/mesh_faces_np_{index}.npy", mesh_faces_np)
            else:
                mesh_vertices_np = np.load(mesh_vertices_np_path)
                mesh_faces_np = np.load(mesh_faces_np_path)
                
                # import open3d as o3d
                # mesh_pcd = o3d.geometry.PointCloud()
                # mesh_pcd.points = o3d.utility.Vector3dVector(mesh_vertices_np)
                # o3d.io.write_point_cloud(f"mesh_pcd_{index}.ply", mesh_pcd)

                # point_cloud_pcd = o3d.geometry.PointCloud()
                # point_cloud_pcd.points = o3d.utility.Vector3dVector(point_cloud)
                # o3d.io.write_point_cloud(f"point_cloud_pcd_{index}.ply", point_cloud_pcd)
            ...
            if len(self.cache) < self.cache_size:
                self.cache[index] = (point_cloud, traj, traj_as_pc, stroke_ids, stroke_ids_as_pc, dirname, extra_items, mesh_vertices_np, mesh_faces_np)
...
class Paintnet_ODv1_CollateBatch(object):
    ...
    def __call__(self, data):
        ...
        # Handle mesh data
        mesh_vertices_np = [d.get('mesh_vertices_np', None) for d in data]
        mesh_faces_np = [d.get('mesh_faces_np', None) for d in data]
